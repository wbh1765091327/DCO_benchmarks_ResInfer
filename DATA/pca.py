...
if __name__ == "__main__":
    ...
    print(f"PCA - {dataset}")
    # path
    path = os.path.join(source, f'_{dataset}')
    base_path = os.path.join(path, f'_{dataset}_base.fvecs')
    ground_path = os.path.join(path, f'_{dataset}_learn_groundtruth.ivecs')
    # read data vectors
    ...
    save_base_path = f'./DATA/_{dataset}/_{dataset}_base_pca.fvecs'
    matrix_save_path = f'./DATA/_{dataset}/_{dataset}_pca_matrix.fvecs'
    ...
    for K in [10, 100]:
        matrix_save_path = f'./DATA/_{dataset}/_{dataset}_pca_matrix_{K}.fvecs'
