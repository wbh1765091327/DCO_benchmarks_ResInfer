...
static void test_approx(float *massQ, size_t vecsize, size_t qsize, HierarchicalNSW<float> &appr_alg, size_t vecdim,
    ...
    for (int i = 0; i < qsize; i++) {
        ...
        std::vector<labeltype> pruned_labels;
#endif
#ifndef WIN32
...
#endif
#ifdef COUNT_PRUNING_STATS
        std::priority_queue<std::pair<float, labeltype >> result = appr_alg.searchKnnWithPruningStats(massQ + vecdim * i, k, adaptive, &pruned_labels);
#else
...
#endif
#ifdef COUNT_PRUNING_STATS
        // 验证剪枝正确性：检查被剪枝的对象是否真的不在ground truth中
        if (!pruned_labels.empty()) {
            ...
            // 检查每个被剪枝的对象
            for (labeltype pruned_label : pruned_labels) {
                if (gt_set.find(pruned_label) == gt_set.end()) {
...
#ifdef COUNT_PRUNING_STATS
    unsigned long long false_pruned = adsampling::tot_pruned_objects - adsampling::correct_pruned_objects;
    
    // 错误剪枝率（万分之几）
    double false_pruning_rate_per_10k = (adsampling::tot_pruned_objects > 0) ? 
        (double)false_pruned / adsampling::tot_pruned_objects * 10000.0 : 0.0;
    
    // 平均每查询错误剪枝数
    double false_pruning_per_query = (double)false_pruned / qsize;
    
    // 错误剪枝与KNN的比例（百分之几）
    double false_pruning_vs_knn_permille = (double)false_pruned / (qsize * k) * 100.0;
    
    cout << appr_alg.ef_ << " " 
         << recall * 100.0 << " " 
         << time_us_per_query << " " 
         << adsampling::tot_pruned_objects << " " 
         << false_pruned << " "                        // 错误剪枝绝对数量
         << false_pruning_rate_per_10k << " "          // 错误剪枝率(万分之几)
         << false_pruning_per_query << " "             // 平均每查询错误剪枝数
         << false_pruning_vs_knn_permille << endl;     // 错误剪枝/KNN比例(百分之几)
#else  
    cout << appr_alg.ef_ << " " 
         << recall * 100.0 << " " 
         << time_us_per_query << " " 
         << total_time / 1e6 << " "               // 总时间
         << adsampling::sqr_dist_time / 1e6 << " "      // 准备时间
         << endl; 
#endif
...
static void test_vs_recall(float *massQ, size_t vecsize, size_t qsize, HierarchicalNSW<float> &appr_alg, size_t vecdim,
    ...
    float recall_threshold = 1e-4;  // 0.01 = 1%
    ...
    for (size_t ef: efs) {
        ...
        // 检查早停条件：recall差值小于阈值
        if (outer_recall - prev_recall < recall_threshold) {
            cout << "Early stopping: recall improvement < " << recall_threshold * 100 << "%" << endl;
            break;
        }
        
        // 更新上次recall值
        prev_recall = outer_recall;
