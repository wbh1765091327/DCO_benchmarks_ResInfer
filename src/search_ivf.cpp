...
void test(const Matrix<float> &Q, const Matrix<unsigned> &G, const IVF &ivf, int k) {
    ...
    for (auto nprobe: nprobes) {
        ...
        for (int i = 0; i < Q.n; i++) {
            ...
            for(int j = 0; j < k; j++) {
                gt_for_query.push_back(G.data[i * G.d + j]);
            }
#endif
            ResultHeap KNNs;
            ...
            if (randomize <= 2)
#ifdef COUNT_PRUNING_STATS
                KNNs = ivf.searchWithPruningStats(Q.data + i * Q.d, k, nprobe, gt_for_query);
#else
                KNNs = ivf.search(Q.data + i * Q.d, k, nprobe);
#endif
            else if (randomize == 3)
#ifdef COUNT_PRUNING_STATS
                KNNs = ivf.search_with_learned_pca_l2_WithPruningStats(Q.data + i * Q.d, k, nprobe, gt_for_query);
#else
...
#ifdef COUNT_PRUNING_STATS
                KNNs = ivf.search_with_pca_WithPruningStats(Q.data + i * Q.d, k, nprobe, gt_for_query);
#else
                KNNs = ivf.search_with_pca(Q.data + i * Q.d, k, nprobe);
#endif
            else if (randomize == 6) {
#ifdef USE_AVX512
                KNNs = ivf.search_with_quantizer_avx512(Q.data + i * Q.d, k, nprobe);
#else
        ...
        double false_pruning_per_query = (double)false_pruned / Q.n;
        double false_pruning_vs_knn_permille = (double)false_pruned / (Q.n * k) * 100.0;
        ...
        cout << nprobe << " " 
             ...
             << false_pruned << " " 
             << false_pruning_rate_per_10k << " " 
             << false_pruning_per_query << " " 
             << false_pruning_vs_knn_permille << endl;
#else
        cout << nprobe << " " 
        << recall * 100.0 << " " 
        << time_us_per_query << " " 
        ...
        << endl; 
#endif
        if (recall * 100.00 > 99.5 || recall - old_recall < 1e-4) break;
        old_recall = recall;
    }
}
