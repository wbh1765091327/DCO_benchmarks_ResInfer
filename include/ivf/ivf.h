...
public:
...
#ifdef USE_AVX512
    ResultHeap search_with_quantizer_avx512(float *query, size_t k, size_t nprobe,
                                            float distK = std::numeric_limits<float>::max()) const;
#endif
    ...
    // 带剪枝统计的重载版本声明
    ResultHeap searchWithPruningStats(float *query, size_t k, size_t nprobe, 
                                      const std::vector<int>& gt_for_query,
                                      float distK = std::numeric_limits<float>::max()) const;
    
    ResultHeap search_with_pca_WithPruningStats(float *query, size_t k, size_t nprobe, 
                                                const std::vector<int>& gt_for_query,
                                                float distK = std::numeric_limits<float>::max()) const;
    
    ResultHeap search_with_learned_pca_l2_WithPruningStats(float *query, size_t k, size_t nprobe, 
                                                           const std::vector<int>& gt_for_query,
                                                           float distK = std::numeric_limits<float>::max()) const;
    ResultHeap search_with_quantizer_WithPruningStats(float *query, size_t k, size_t nprobe,
                                                      const std::vector<int>& gt_for_query,
                                                      float distK = std::numeric_limits<float>::max()) const;
#endif
};
...
#ifdef COUNT_PRUNING_STATS
// 1. 基础search函数的剪枝统计版本
ResultHeap IVF::searchWithPruningStats(float *query, size_t k, size_t nprobe, 
                                       const std::vector<int>& gt_for_query,
                                       float distK) const {
    Result *centroid_dist = new Result[C];

    // 构建ground truth集合用于快速查找
    std::unordered_set<int> gt_set(gt_for_query.begin(), gt_for_query.end());
    ...
    if (d < D) {
        ...
        for (int i = 0; i < nprobe; i++) {
            ...
            for (int j = 0; j < len[cluster_id]; j++) {
                ...
                if (tmp_dist < 0) {
                    // 被剪枝的对象
                    int object_id = id[can];
                    adsampling::tot_pruned_objects++;
                    
                    // 验证剪枝正确性
                    if (gt_set.find(object_id) == gt_set.end()) {
                        adsampling::correct_pruned_objects++;
                    }
                } else {
                    // 通过剪枝测试的对象
                    KNNs.emplace(tmp_dist, id[can]);
...
// 2. PCA版本的剪枝统计
ResultHeap IVF::search_with_pca_WithPruningStats(float *query, size_t k, size_t nprobe, 
                                                 const std::vector<int>& gt_for_query,
                                                 float distK) const {
    Result *centroid_dist = new Result[C];
    PCA->get_query_square(query);
    ...
    if (d < D) {
        ...
        for (int i = 0; i < nprobe; i++) {
            ...
            for (int j = 0; j < len[cluster_id]; j++) {
                size_t can = start[cluster_id] + j;
                float tmp_dist = PCA->uniform_fast_inference(query + d, res_data + can * (D - d), distK, d, *cur_dist);
...
#ifdef COUNT_PRUNING_STATS
ResultHeap IVF::search_with_learned_pca_l2_WithPruningStats(float *query, size_t k, size_t nprobe, 
                                                           const std::vector<int>& gt_for_query,
                                                           float distK) const {
    Result *centroid_dist = new Result[C];
    ...
    for (int i = 0; i < nprobe; i++) {
        ...
        for (int j = 0; j < len[cluster_id]; j++) {
            size_t can = start[cluster_id] + j;
            float tmp_dist = PCA->learned_fast_inference_l2(query + d, res_data + can * (D - d), distK, 1, *cur_dist);
...
ResultHeap IVF::search_with_quantizer(float *query, size_t k, size_t nprobe, float distK) const {
    ...
    while (!CoarseQ.empty()) {
        ...
        CoarseQ.pop();
        // node_cluster_dist_是原始数据向量 can 与 其自身的量化版本 PQ(can) 之间的距离
...
#ifdef USE_AVX512
ResultHeap IVF::search_with_quantizer_avx512(float *query, size_t k, size_t nprobe, float distK) const {
    // the default value of distK is +inf
    Result *centroid_dist = new Result[C];
...
// 使用码本对数据向量进行编码，计算每个数据向量与码本中每个码字的距离，并记录每个数据向量与码本中最近码字的距离
void IVF::encoder_origin_data() {
    // pq_mp存储所有数据向量的 PQ 编码。
    PQ->pq_mp = new unsigned char[PQ->nd_ * PQ->sub_vector];
    // node_cluster_dist_存储所有数据向量与码本中最近码字的距离。
...
#ifdef COUNT_PRUNING_STATS
ResultHeap IVF::search_with_quantizer_WithPruningStats(float *query, size_t k, size_t nprobe,
                                                       const std::vector<int>& gt_for_query,
                                                       float distK) const {
    Result *centroid_dist = new Result[C];
    ...
    while (!CoarseQ.empty()) {
        ...
        // Not pruned, calculate exact distance and update KNNs
        float tmp_dist = sqr_dist(query, L1_data + can * D, D);
        if (KNNs.size() < k) {
            KNNs.emplace(tmp_dist, id[can]);
            if (KNNs.size() == k) thresh = KNNs.top().first;
        }
        else if (tmp_dist < thresh) { // thresh is KNNs.top().first
            KNNs.pop();
            KNNs.emplace(tmp_dist, id[can]);
            thresh = KNNs.top().first;
        }
