...
namespace finger {
    ...
    void pre_compute_cos() {
        cos_table.resize(65);
        for (float i = 0; i <= 64; i++) {
            cos_table[i] = cos(i / ((float) lsh_dim) * M_PI);
        }
    }

    float look_up_dist_comp(const float &bsf, const float &t, const float &b, const float &c_2,
                            const float &d_res, const float &q_res, const float &q_res_2,
                            const unsigned long long &binary_sgn_q_res_P,
                            const unsigned long long &binary_sgn_d_res_P) {
        auto tmp = t - b;
        p1 = tmp * tmp * c_2;
        p2 = d_res * d_res;
        p3 = q_res_2;
        cos_theta = cos_table[__builtin_popcountll(binary_sgn_q_res_P ^ binary_sgn_d_res_P)];
        p4 = -2 * q_res * d_res * cos_theta;
