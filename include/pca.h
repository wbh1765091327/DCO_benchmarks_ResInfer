    ...
    public:
        ...
        void get_query_square(const float *q) {
            ...
            for (int i = 0; i < dimension_; i++) {
                query_square += q[i] * q[i]; // 计算查询向量的L2范数平方
                pre_query[i] = q[i] * q[i] * var[i];// 每个维度乘以对应的方差权重
            }
            pre_query[dimension_] = 0;
            for (int i = (int) dimension_ - 1; i >= 0; i--) {
                pre_query[i] += pre_query[i + 1]; // 计算从第i维到最后一维的累积加权平方和（计算从第i维到最后一维的总贡献）
            }
            for (int i = 0; i < dimension_; i++) {
                pre_query[i] = sqrt(pre_query[i]); // 开平方根
                pre_query[i] *= sigma_count * 2; // 乘以置信度参数：从第i维到最后一维的累积加权平方和的平方根，乘以置信度参数
            }
        }
