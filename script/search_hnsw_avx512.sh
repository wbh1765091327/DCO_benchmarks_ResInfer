...
for K in 10; do
  for data in "${datasets[@]}"; do
    ...
    result_path="./results/recall@${K}/hnsw/simd/${data}"
    temp_data=./DATA/${data}
    ...
    #   ./build/src/search_hnsw_avx512_dist -d ${randomize} -p ${delta_d} -i ${index} -q ${query} -g ${gnd} -r ${res1} -t ${trans} -k ${K} -s ${efSearch} 
    #   # ./build/src/search_hnsw_avx512_pruning -d ${randomize} -p ${delta_d} -i ${index} -q ${query} -g ${gnd} -r ${res2} -t ${trans} -k ${K} -s ${efSearch}
    ...
    # ./build/src/search_hnsw_avx512_dist -d ${randomize} -p ${delta_d} -i ${index} -q ${query} -g ${gnd} -r ${res1} -t ${trans} -l ${linear} -k ${K} -s ${efSearch} 
    # # ./build/src/search_hnsw_avx512_pruning -d ${randomize} -p ${delta_d} -i ${index} -q ${query} -g ${gnd} -r ${res2} -t ${trans}  -l ${linear} -k ${K} -s ${efSearch}
    ...
    # res1="${result_path}/${data}_ad_hnsw_${randomize}_dist_time.log"
    # res2="${result_path}/${data}_ad_hnsw_${randomize}_pruning_stats.log"
    # ./build/src/search_hnsw_avx512_dist -d ${randomize} -p ${delta_d} -i ${index} -q ${query} -g ${gnd} -r ${res1} -t ${trans} -e ${sigma} -k ${K} -s ${efSearch} 
    # # ./build/src/search_hnsw_avx512_pruning -d ${randomize} -p ${delta_d} -i ${index} -q ${query} -g ${gnd} -r ${res2} -t ${trans} -e ${sigma} -k ${K} -s ${efSearch}
    
    index="${index_path}/${data}_ef500_M16_opq.index"
    trans="${temp_data}/${data}_opq_matrix.fvecs"
    randomize=4
    code="${temp_data}/${data}_codebook.centroid"
    res1="${result_path}/${data}_ad_hnsw_${randomize}_simd_dist_time.log"
    res2="${result_path}/${data}_ad_hnsw_${randomize}_pruning_stats.log"
    linear="${index_path}/linear/linear_hnsw1_opq_${K}.log"
    ./build/src/search_hnsw_avx512_dist -d ${randomize} -i ${index} -q ${query} -b ${code} -g ${gnd} -r ${res1} -t ${trans} -l ${linear} -k ${K} -s ${efSearch} 
    # ./build/src/search_hnsw_avx512_pruning -d ${randomize} -i ${index} -q ${query} -b ${code} -g ${gnd} -r ${res2} -t ${trans} -l ${linear} -k ${K} -s ${efSearch}
  done
done
